  ...
  pbbs::launch(argc, argv, [&] (pbbs::measured_type measured) {
    ...
    if (infile != "") {
      ...
      d.add("array_point2d", [&] {
        ...
        pbbs_pctl_call<_point2d<double>, pbbs::_point2d<double>, 10>(measured, x, k);
      });
      d.add("array_point3d", [&] {
        parray<_point3d<double>> x = pasl::pctl::io::load<parray<_point3d<double>>>(infile);
        pbbs_pctl_call<_point3d<double>, pbbs::_point3d<double>, 10>(measured, x, k);
      });                                                                           
      d.dispatch("type");
      return;
    }
    ...
    if (test == 0) {
      ...
      pbbs_pctl_call<_point2d<double>, pbbs::_point2d<double>, 1>(measured, a, 1);
    } else if (test == 1) {
      parray<_point2d<double>> a;
      if (files) {
      ...
      pbbs_pctl_call<_point3d<double>, pbbs::_point3d<double>, 1>(measured, a, 1);
    } else if (test == 3) {
      parray<_point3d<double>> a;
      if (files) {
      ...
      pbbs_pctl_call<_point3d<double>, pbbs::_point3d<double>, 10>(measured, a, 10);
    } else if (test == 5) {
      parray<_point3d<double>> a;
      if (files) {
        a = pasl::pctl::io::load_seq_from_txt<_point3d<double>>(std::string("tests/random_in_cube_3d_txt_10000000"), path_to_data + std::string("3DinCube_10M"), 10000000, reload);
